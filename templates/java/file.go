...
public class {{ classNameFile . }}Validator {
	public static cn.spaceli.pgv.ValidatorImpl validatorFor(Class clazz) {
		{{ range .AllMessages }}
		{{ if not (ignored .) -}}
...
*/
@SuppressWarnings("all")
public class {{ classNameMessage .}}Validator implements cn.spaceli.pgv.ValidatorImpl<{{ qualifiedName . }}>{
	public static cn.spaceli.pgv.ValidatorImpl validatorFor(Class clazz) {
		if (clazz.equals({{ qualifiedName . }}.class)) return new {{ simpleName .}}Validator();
		{{ range .AllMessages }}
		{{ if not (ignored .) -}}
